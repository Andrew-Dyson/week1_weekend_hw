...
def get_pets_by_breed(requested_pet_shop, requested_breed):
    matched_breeds = []
    for pet in requested_pet_shop["pets"]:
        if pet["breed"] == requested_breed:
            matched_breeds.append(pet)
    return matched_breeds
...
def find_pet_by_name(requested_pet_shop, pet_name_to_search):
    ...
    for pet in pets:
        if pet["name"] == pet_name_to_search:
            return pet   
        # else: 
        #     return None     # Gives error - forces you out of loop and returns None too early
...
# solution from cc instructor
def customer_can_afford_pet(customer_to_check_affordability, potential_new_pet):
    return customer_to_check_affordability["cash"] >= potential_new_pet["price"]
...
#Solution from cc instructor:
def sell_pet_to_customer(requested_pet_shop, pet_to_sell, customer_requesting_pet):
    if pet_to_sell != None and customer_can_afford_pet(customer_requesting_pet, pet_to_sell):
        remove_pet_by_name(requested_pet_shop, pet_to_sell["name"])
        add_pet_to_customer(customer_requesting_pet, pet_to_sell)
        add_or_remove_cash(requested_pet_shop, pet_to_sell["price"])
        remove_customer_cash(customer_requesting_pet, pet_to_sell["price"])
        increase_pets_sold(requested_pet_shop, len(customer_requesting_pet["pets"]))